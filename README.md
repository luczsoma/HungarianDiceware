...
If elements are chosen randomly from a set of $N$ symbols, the entropy per symbol ($\Eta_s$) in bits is given by the formula $\Eta_s = \log_2 N$. Per-symbol entropy values for common symbol sets are summarized in the table below.

| Symbol set                                                 | Symbol count ($N$) | Entropy per symbol ($\Eta_s$) in bits |
| ---------------------------------------------------------- | ------------------ | ------------------------------------- |
...
With a password generated randomly from a set of $N$ symbols, the minimum length of the password ($L$) required to achieve a desired password entropy ($\Eta$) is given by the formula $$L = \left\lceil \frac{\Eta}{\log_2 N} \right\rceil,$$ where $\lceil~\rceil$ denotes the mathematical ceiling function (i.e., mapping $x$ to the least integer greater than or equal to $x$). For the given levels of desired password entropy, the minimum required length values for common symbol sets are summarized in the table below.

| Desired password entropy ($\Eta$) | Case sensitive Latin alphabet ($N = 52$) | Case sensitive alphanumeric characters ($N = 62$) | ASCII printable characters ($N = 95$) | Extended ASCII printable characters ($N = 218$) | Diceware word list ($N = 7776$) |
| --------------------------------- | ---------------------------------------- | ------------------------------------------------- | ------------------------------------- | ----------------------------------------------- | ------------------------------- |
...
Let $L_1$ be the set of the 35 letters that can be present in a passphrase created with Diceware using the Version 2 list. These are the lowercase letters of the extended Hungarian alphabet excl. double and triple letters (‘cs’, ‘dz’, ‘dzs’, ‘gy’, ‘ly’, ‘ny’, ‘sz’, ‘ty’, ‘zs’): $$L_1 = \{ \mathrm{a}, \mathrm{á}, \mathrm{b}, \mathrm{c}, \mathrm{d}, \mathrm{e}, \mathrm{é}, \mathrm{f}, \mathrm{g}, \mathrm{h}, \mathrm{i}, \mathrm{í}, \mathrm{j}, \mathrm{k}, \mathrm{l}, \mathrm{m}, \mathrm{n}, \mathrm{o}, \mathrm{ó}, \mathrm{ö}, \mathrm{\H{o}}, \mathrm{p}, \mathrm{q}, \mathrm{r}, \mathrm{s}, \mathrm{t}, \mathrm{u}, \mathrm{ú}, \mathrm{ü}, \mathrm{\H{u}}, \mathrm{v}, \mathrm{w}, \mathrm{x}, \mathrm{y}, \mathrm{z} \}$$

Let $i_1$ be a function that maps elements of $L_1$ to their numeric position in the Hungarian alphabetically ordered enumeration of $L_1$, such that $$i_1(\mathrm{a}) = 1,$$ $$i_1(\mathrm{á}) = 2,$$ $$\ldots$$ $$i_1(\mathrm{z}) = 35.$$
...
$$P(X_1 = i_1(\mathrm{\H{o}})) = \frac{TODO}{TODO} \approx TODO = TODO\%$$
...
$$P(X_1 = i_1(\mathrm{\H{u}})) = \frac{TODO}{TODO} \approx TODO = TODO\%$$
...
Let $\Eta_1$ be the entropy of $X_1$ in bits: $$\Eta_1 := \Eta(X_1) = - \sum_{l \in L_1} P(X_1 = i_1(l)) \log_2 P(X_1 = i_1(l)) \approx TODO~\mathrm{bit}$$

Let $L_2$ be the set of the 26 letters that can be present in a passphrase created with Diceware using the Version 2 list, if substituting accented letters with their non-accented variants according to the following substitution rules (the sign ‘>’ denoting substitution): ‘á’ > ‘a’, ‘é’ > ‘e’, ‘í’ > ‘i’, ‘ó’ > ‘o’, ‘ö’ > ‘o’, ‘ő’ > ‘o’, ‘ú’ > ‘u’, ‘ü’ > ‘u’, ‘ű’ > ‘u’. These are the lowercase letters of the English alphabet: $$L_2 = \{ \mathrm{a}, \mathrm{b}, \mathrm{c}, \mathrm{d}, \mathrm{e}, \mathrm{f}, \mathrm{g}, \mathrm{h}, \mathrm{i}, \mathrm{j}, \mathrm{k}, \mathrm{l}, \mathrm{m}, \mathrm{n}, \mathrm{o}, \mathrm{p}, \mathrm{q}, \mathrm{r}, \mathrm{s}, \mathrm{t}, \mathrm{u}, \mathrm{v}, \mathrm{w}, \mathrm{x}, \mathrm{y}, \mathrm{z} \}$$

Let $i_2$ be a function that maps elements of $L_2$ to their numeric position in the alphabetically ordered enumeration of $L_2$, such that $$i_2(\mathrm{a}) = 1,$$ $$i_2(\mathrm{b}) = 2,$$ $$\ldots$$ $$i_2(\mathrm{z}) = 26.$$

Let $l \in L_2$ be a letter chosen randomly from the Version 2 list, substituting if accented by the rules above, and let $X_2$ be a random variable that represents $i_2(l)$. $X_2$ has the following probability distribution:
...
Let $\Eta_2$ be the entropy of $X_2$ in bits: $$\Eta_2 := \Eta(X_2) = - \sum_{l \in L_2} P(X_2 = i_2(l)) \log_2 P(X_2 = i_2(l)) \approx TODO~\mathrm{bit}$$
...
The minimum letter count requirement follows from the fact that one does not want to lose any security by considering their Diceware passphrase as a string of independent characters present in a particular Diceware word list. That is, one does not want to be less secure in case an attacker — in possession of the information that one uses Diceware with a particular word list — decides to start brute-force cracking one’s passphrase not word by word, but character by character, based on the probability distribution of the used word list’s characters. This can be achieved by ensuring that the passphrase’s “character-based” entropy ($\Eta_\mathrm{C}$) is at least as much as its “word-based” entropy ($\Eta_\mathrm{W}$): $$\Eta_\mathrm{C} \geq \Eta_\mathrm{W}.$$

The “word-based” entropy bits of a Diceware passphrase containing $W$ words is given by the formula $\Eta_\mathrm{W} = W \cdot \log_2 7776$. The “character-based” entropy bits of a Diceware passphrase is given by the formula $\Eta_\mathrm{C} = L \cdot \Eta_\mathrm{c}$, where $$

The “character-based” entropy of a Diceware passphrase generated using the Version 2 list in the worst case (neither using word separators in the passphrase nor decorating it in any way) is $\Eta_1 \approx TODO~\mathrm{bit}$ if using accented letters and $\Eta_2 \approx TODO~\mathrm{bit}$ if not using accented letters.

The “word-based” per-symbol entropy ($\Eta_\mathrm{w}$) of a Diceware passphrase is $\Eta_\mathrm{w} \approx 12.925~\mathrm{bit}$, meaning that the “word-based” entropy of a Diceware passphrase is $\Eta_\mathrm{W} = W \cdot \Eta_\mathrm{w}$. This means that each word in the passphrase needs to have at least ~12.925 bits of “character-based” entropy as well, in order not to weaken the overall security of the passphrase. The “character-based” per-symbol entropy of a Diceware passphrase generated using the Version 2 list in the worst case (neither using word separators in the passphrase nor decorating it) is $\Eta_1 \approx TODO~\mathrm{bit}$ if using accented letters and $\Eta_2 \approx TODO~\mathrm{bit}$ if not using accented letters.

With a Diceware passphrase containing $W$ words without separators and any decoration, the passphrase’s minimum number of letters ($L$), required to achieve at least the same “character-based” entropy as the “word-based” entropy ($\Eta_\mathrm{W} = W \cdot \Eta_\mathrm{w}$) of the passphrase, is given by the formula $$L = \left\lceil \frac{W \cdot \Eta_\mathrm{w}}{\Eta_\mathrm{c}} \right\rceil,$$ where $\lceil~\rceil$ denotes the mathematical ceiling function (i.e., mapping $x$ to the least integer greater than or equal to $x$), and $\Eta_\mathrm{c}$ is the “character-based” entropy either H<sub>c<sub>1</sub></sub> if using accented characters or H<sub>c<sub>2</sub></sub> if not using accented characters. Values of L for 5 <= W <= 20 with H<sub>c<sub>1</sub></sub> = TODO and H<sub>c<sub>2</sub></sub> = TODO are summarized in the table in the “How to use it?” section.
